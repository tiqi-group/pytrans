...
trap = Trap()
well0 = PotentialWell(0, depth, axial, split, tilt, freq_pseudo=trap.freq_pseudo, scale_roi=4)
# well = PotentialWell(x0, depth, axial, split, tilt, freq_pseudo=trap.freq_pseudo, scale_roi=4)
wells = [well0]
n_wells = len(wells)
...
voltages = solver.static_solver(r0=0, rx=1, rh=0)
