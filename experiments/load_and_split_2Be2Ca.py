...
def load_and_split_2Be2Ca(add_reordering=True, analyse_wfms=False):
    ...
    try:
        ...
        wfs_load_and_split_2Be2Ca = WaveformSet(waveform_file=wf_path)
        ...
        n_transport = 308
        load_to_split, wf_split = split_waveforms_2Be2Ca(0, freq_2Be2Ca, offs_2Be2Ca,
                                                         [-844, 0], [freq_BeCa,freq_BeCa], [offs_BeCa, offs_BeCa],
                                                         -422.5, freq_2Be2Ca, None,
                                                         n_transport=n_transport,
                                                         electrode_subset=[3,4,5,6,7,18,19,20,21,22]) # left splitting group
        wfs_load_and_split_2Be2Ca.waveforms.append(load_to_split)
        wf_far_to_exp = lc.transport_waveform_multiple(
            [[-844,0],[0,600]],
            [[freq_BeCa, freq_BeCa],[freq_BeCa, freq_BeCa]],
            [[offs_BeCa,offs_BeCa],[offs_BeCa,offs_BeCa]],
            2.5*n_transport,
        ...
        wfs_load_and_split_2Be2Ca.waveforms.append(wf_split)
        wfs_load_and_split_2Be2Ca.waveforms.append(wf_far_to_exp)
        wfs_load_and_split_2Be2Ca.waveforms.append(wf_recombine_fast)
                    
        wfs_load_and_split_2Be2Ca.write(wf_path)

        animate_split = True
        if animate_split:
            animate_wavpots([WavPotential(k) for k in (load_to_split, wf_split, wf_far_to_exp)],
                            parallel=False, decimation=1)# , save_video_path='load_and_split_2Be2Ca.mp4')

    # Create a single testing waveform, made up of the individual transports
    add_testing_waveform = False
    if add_testing_waveform:
        test_waveform_present = wfs_load_and_split_2Be2Ca.get_waveform(-1).desc == "trans + split, then reverse"
        if test_waveform_present:
            exp_to_split_wfm = wfs_load_and_split_2Be2Ca.get_waveform(-4)
            split_wfm = wfs_load_and_split_2Be2Ca.get_waveform(-3)
            far_to_cent_wfm = wfs_load_and_split_2Be2Ca.get_waveform(-2)
        else:
            exp_to_split_wfm = wfs_load_and_split_2Be2Ca.get_waveform(-3)
            split_wfm = wfs_load_and_split_2Be2Ca.get_waveform(-2)
            far_to_cent_wfm = wfs_load_and_split_2Be2Ca.get_waveform(-1)            
        ...
        if test_waveform_present:
            wfs_load_and_split_2Be2Ca.waveforms[-1] = wf_trans_split_for_rev
        else:
            wfs_load_and_split_2Be2Ca.waveforms.append(wf_trans_split_for_rev)
