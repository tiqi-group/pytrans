...
def vSet_axial(curv):
    ...
    # volt_axial = [-0.25685, 0, 0, 0.34665, 0.34665, -1.6753, 0.34665, 0.34665, 0, 0,
    #               -0.25685, 0, 0, 0.34665, 0.34665, -1.6753, 0.34665, 0.34665, 0, 0,
    #               0, 0, 0, 0, 0]

    # v_ax2
    volt_axial = [-0.248188, 0, 0, 6.5633, -3.27655, -1.0577, -3.27655, 6.5633, 0, 0,
                  -0.248188, 0, 0, 6.5633, -3.27655, -1.0577, -3.27655, 6.5633, 0, 0,
                  0, 0, 0, 0, 0]
...
def calculate_voltage(curv, tilt, xComp, yComp, zComp):  # , xCubic, vMesh, vGND, xyTilt=0, xzTilt=0):
    # Array of voltages. 20 electrodes + mesh + (GND level)*4
    voltages = np.zeros(25)
    # # Sum up all contributions
    voltages = (vSet_axial(curv) +
                vSet_tilt(tilt) +
                vSet_xComp(xComp) +
                vSet_yComp(yComp) +
                vSet_zComp(zComp)
