...
class _old_AbstractTrap:
    ...
    def setup_Vmaxmins(self):
        def V_to_list(V):
            if (isinstance(V, list)):
                ...
                return np.array(self.numberofelectrodes * [V])
    ...
    def setuptrapaxisBSpline(self, s=3.2e-10):
        self.trapAxisSplineInterpol = []  # np.array([] * self.numberofelectrodes)
    ...
    def FuncbyBspline(self, x, deriv):
        res = np.zeros(self.numberofelectrodes)
        for i in range(self.numberofelectrodes):
